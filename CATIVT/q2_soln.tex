...
For the following function $g$ defined by
	...
	\dfrac{(t-1)(t+2)}{t+2}	&	\qquad \text{if } -3 \leq t < 1 \text{ and } t \neq -2	\\ \\
	4 \ln t				&	\qquad \text{if } t \geq 1					\end{array} \right.  $

  find the \textbf{Intervals of Continuity}.  \\
  (\textbf{Important Note:} Write your answer as a list of intervals, with each interval \textbf{separated by a comma}.)	
...
Although we have found $g$ is not continuous at $t = -3$, since $\lim_{t \to -3^+} g(t)=g(-3)$, $g$ is continuous from the right at $t=-3$.
  ...
  The function $g$ is continuous on $[-3, -2),(-2,1)$ since, in this interval, $g(t) = \dfrac{(t-1)(t+2)}{t+2}$ is a rational function whose denominator is not zero. $g$ is not continuous at $t=-2$ because it is not defined there.   $g$ is continuous on $(1, \infty)$, since, on this interval, $g(t) =  4 \ln(t)$ is a constant multiple of a  logarithmic function.
