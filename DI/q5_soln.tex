...
\begin{problem}
  ...
  \begin{enumerate}
    \item
    If $f$ is an odd function, why is it true that $\displaystyle \int_{-a}^a f(x)
    \d x = 0$?  
...
\item
      If $f$ is an even function, why is it true that $\displaystyle \int_{-a}^a f(x) \d x = 2 \displaystyle \int_0^a f(x) \d x$?
      Support your reasoning with a picture.
      \begin{explanation}
